...
def normalize_image(image_bytes):
    """
    Converts any input image format into a standard RGB JPEG format.
    """
    try:
...
def extract_text_with_paddleocr(ordered_image_bytes):
    """Step 1: Use PaddleOCR for high-accuracy raw text extraction."""
    full_text = ""
...
def structure_data_with_specialist_llm(raw_text, base64_images, doc_type_hint):
    """Step 2: Use a powerful multimodal LLM with the specialist prompt."""
    prompt = f"""
    You are an OCR engine specialized in identity documents...
    (The rest of the large professional prompt goes here)
    """
    # (Full prompt text omitted for brevity, but should be the one you provided)
    try:
...
def process_documents_task(self, file_contents_dict, doc_type):
    """The main Celery task orchestrating the universal pipeline."""
    try:
        all_image_bytes = []
        original_images_to_save = []

        # Unpack the file contents, processing PDFs into images
        ...
        # --- Intelligent Correction & Structuring ---
